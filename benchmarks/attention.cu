...
template<int Br_, int Bc_,int Kd_, int D_, int Nthreads_, int BlockKSmem_=Kd_, int num_stages_qk_=1, int BlockKSmem2_=Bc_, int num_stages_v_=1, int SmemKAtom_=64, int kSwizzle_=3, bool unrollLastIter_=true>
...
public:
    ...
    constexpr static int kSwizzle = kSwizzle_;
...
float test_attention(ProblemShape shape){
    ...
    auto kernel = &flashattn_fwd<kd,d,br,bc,Nthreads,BlockKSmem,num_stages_qk,load_q_once,BlockKSmem2,num_stages_v,SmemKAtom,kSwizzle,unrollLastIter>;
...
int main(){
    ...
    float ms = test_attention<InpleConfig>(PS);
